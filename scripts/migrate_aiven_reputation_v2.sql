-- Migración del sistema de reputación para Aiven (versión compatible)
USE astrengpr;

-- Verificar y agregar columnas faltantes a la tabla tareas
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS 
     WHERE TABLE_SCHEMA = 'astrengpr' 
     AND TABLE_NAME = 'tareas' 
     AND COLUMN_NAME = 'fecha_completada') = 0,
    'ALTER TABLE tareas ADD COLUMN fecha_completada DATETIME DEFAULT NULL',
    'SELECT "fecha_completada ya existe" as message'
));
...
SET @sql = (SELECT IF(
    ...
    'ALTER TABLE tareas ADD COLUMN fecha_reapertura DATETIME DEFAULT NULL',
    'SELECT "fecha_reapertura ya existe" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- Agregar índices de rendimiento (verificar si existen primero)
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = 'astrengpr' 
     AND TABLE_NAME = 'tareas' 
     AND INDEX_NAME = 'idx_tareas_usuario_estado_completada') = 0,
    'CREATE INDEX idx_tareas_usuario_estado_completada ON tareas (usuario_id, estado, fecha_completada)',
    'SELECT "idx_tareas_usuario_estado_completada ya existe" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE TABLE_SCHEMA = 'astrengpr' 
     AND TABLE_NAME = 'tareas' 
     AND INDEX_NAME = 'idx_tareas_area_estado_completada') = 0,
    'CREATE INDEX idx_tareas_area_estado_completada ON tareas (area_id, estado, fecha_completada)',
    'SELECT "idx_tareas_area_estado_completada ya existe" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
